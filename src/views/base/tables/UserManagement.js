...
const UserManagement = () => {
  ...
  const fetchGuides = async () => {
    try {
      const response = await axios.get('http://3.111.163.2:5000/api/guide/');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://3.111.163.2:5000/api/guide/${id}`);
  ...
  const handleAddGuide = async (event) => {
    ...
    try {
      const response = await axios.post('http://3.111.163.2:5000/api/guide/register', formData);
  ...
  const handleEditGuide = async (event) => {
    ...
    try {
      await axios.put(`http://3.111.163.2:5000/api/guide/${_id}`, formData);
